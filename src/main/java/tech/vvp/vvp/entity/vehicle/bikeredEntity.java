...
import tech.vvp.vvp.init.ModEntities; // Предполагается, что сущность регистрируется здесь
...
public class bikeredEntity extends ContainerMobileVehicleEntity implements GeoEntity, LandArmorEntity, ArmedVehicleEntity {
    ...
    // Конструктор для спавна по сети (заменяет статический метод clientSpawn)
    public bikeredEntity(PlayMessages.SpawnEntity packet, Level world) {
        this((EntityType<bikeredEntity>) packet.getEntityType(), world);
    }
    ...
    // Метод travel() удален, его логика теперь в handleVehicleMovement()
